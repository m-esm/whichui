...
COPY --from=main-builder /app/next.config.js ./

# Copy the built MUI app to the public folder
COPY --from=mui-builder /app/uis/mui/dist ./public/mui

# Copy the built Ant Design app to the public folder
COPY --from=ant-builder /app/uis/ant/dist ./public/ant
